...
data class DownloadConfig(
    ...
    var cdnAuthTokens: ConcurrentHashMap<Pair<Int, String>, CompletableDeferred<CDNAuthToken>> =
        ConcurrentHashMap<Pair<Int, String>, CompletableDeferred<CDNAuthToken>>()
)
...
class ContentDownloader(val steamClient: SteamClient, private val config: DownloadConfig) {
    ...
    suspend fun getDepotInfo(depotId: Int, appId: Int, manifestId: Long, branch: String): DepotDownloadInfo? {
        ...
        if (manifestId == INVALID_MANIFEST_ID) {
            ...
            if (manifestId == INVALID_MANIFEST_ID && branch.equals(DEFAULT_BRANCH, ignoreCase = true)) {
        ...
        val depotKey = config.depotKeys.get(depotId)
        if (depotKey == null) {
            ...
            if (newManifest != null) {
                ...
                do {
                    ...
                    try {
                        ...
                        val authTokenCallbackPromise = config.cdnAuthTokens[Pair(depot.depotId, connection!!.host)]
    ...
    suspend fun requestDepotKey(depotId: Int, appId: Int = 0) {
        if (config.depotKeys.containsKey(depotId)) {
            return
        }

        val depotKey = steamClient.getHandler<SteamApps>()!!.getDepotDecryptionKey(depotId, appId).await()
        ...
        config.depotKeys[depotKey.depotID] = depotKey.depotKey
    }
    ...
    suspend fun getDepotManifestRequestCode(
        ...
        val requestCode = steamClient.getHandler<SteamContent>()!!
            .getManifestRequestCode(depotId, appId, manifestId, branch, parentScope = this)
            .await()
