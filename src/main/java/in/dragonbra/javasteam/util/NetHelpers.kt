...
object NetHelpers {
    ...
    fun getIPAddress(ip: InetAddress): Int {
        val buff = ByteBuffer.wrap(ip.address)
        return buff.getInt() and 0xFFFFFFFFL.toInt()
    }

    @JvmStatic
    @Throws(IllegalArgumentException::class)
    fun getLocalIP(activeSocket: Socket): InetAddress? {
        val ipEndPoint = activeSocket.localSocketAddress as? InetSocketAddress

        if (ipEndPoint == null || ipEndPoint.address == InetAddress.getByName("0.0.0.0")) {
            return null
        }
    ...
    @JvmStatic
    @Throws(IllegalArgumentException::class)
    fun getLocalIP(datagramSocket: DatagramSocket): InetAddress? {
        val ipEndPoint = datagramSocket.localSocketAddress as? InetSocketAddress
    ...
    @JvmStatic
    @Throws(IllegalArgumentException::class)
    fun getIPAddressAsLong(ipAddr: InetAddress): Long {
    ...
    fun getMsgIPAddress(ipAddr: InetAddress): CMsgIPAddress {
        ...
        if (ipAddr is Inet6Address) {
            ...
            msgIpAddress.v4 = getIPAddressAsLong(ipAddr).toInt()
        }
